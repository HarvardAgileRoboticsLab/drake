 classdef HybridRobustDirtranTrajectoryOptimization < DirectTrajectoryOptimization
  ...
  methods
    function obj = HybridRobustDirtranTrajectoryOptimization(plant,N,D,E0,Q,R,Qf,duration,options)
      ...
      if ~isfield(options,'mode_sequence')
        error("Please provide a mode sequence!");
      end
   %   if isfield(options,'constant_K')
   %     obj.constant_K = options.constant_K;
   %   end
      if isscalar(duration)
          duration=[duration,duration];
      end

      %obj = HybridTrajectoryOptimization(@DirectTrajectoryOptimization,plant,options.mode_sequence,sum(N_per_mdoe),duration,options);
      obj = obj@DirectTrajectoryOptimization(plant,sum(N),duration,options);
      ...
      obj.N_per_mode = N;
      obj.mode_sequence = options.mode_sequence;
    ...
    function obj = setupVariables(obj, N)
      ...
      %N = sum(obj.N);
      %nH = N-1;
      nH = sum(N) - length(obj.mode_sequence)-1;
      nX = obj.plant.getNumStates()-obj.plant.getNumDiscStates();
    ...
    function z0 = getInitialVars(obj,t_init,traj_init)
        ...
        if isscalar(t_init)
            t_init = linspace(0,t_init,sum(obj.N));
        elseif length(t_init) ~= sum(obj.N)
    ...
    function obj = addDynamicConstraints(obj)
      ...
      nU = obj.plant.getNumInputs();
      N = sum(obj.N);
      %Dynamics constraints are ignored at mode switches. Seperate
      %constraint deals with that
      num_mode_switch = length(obj.N)-1; %Number of times the system switches modes
      num_constraints = N-1 -num_mode_switch;
      
      constraints = cell(num_constraints,1);
      dyn_inds = cell(num_constraints,1);
      ...
      N = [5;5];
      obj.mode_sequence = [1;2];
      dyn_indices = [1:(obj.N(1)-1)];
      mode_num = [obj.mode_sequence(1)*ones(1,length(dyn_indices))];
      
      %Set up indices for dynamics constraint; Between all time-steps
      %except over transitions
      for j = 2:length(obj.N)
        dyn_indices = [dyn_indices,(dyn_indices(end)+2):(dyn_indices(end)+obj.N(j))];
        mode_num = [mode_num,obj.mode_sequence(j)*ones(1,obj.N(j)-1)];
      end
      
      for k=1:length(dyn_indices)
          i = dyn_indices(k);
        switch obj.options.integration_method
          case RobustDirtranTrajectoryOptimization.FORWARD_EULER
            dyn_inds{k} = {obj.h_inds(k);obj.x_inds(:,i);obj.x_inds(:,i+1);obj.u_inds(:,i)};
          case RobustDirtranTrajectoryOptimization.MIDPOINT
            dyn_inds{k} = {obj.h_inds(k);obj.x_inds(:,i);obj.x_inds(:,i+1);obj.u_inds(:,i);mode_num(k)};
          otherwise
            error('Drake:RobustDirtranTrajectoryOptimization:InvalidArgument','Unknown integration method');
        end
        cnstr = cnstr.setName(sprintf('dynamics_constr_%d_',k));
        constraints{k} = cnstr;
        
        obj = obj.addConstraint(constraints{k}, dyn_inds{k});
      end
    end
    ...
    function obj = addRobustCost(obj,Qr,Rr,Qrf)
        ...
        temp_h_inds = zeros(sum(obj.N)-1,1);
        temp_h_inds(1:obj.N(1)-1) = obj.h_inds(1:obj.N(1)-1);
        Ns = [5;5];
        next_jump = Ns(1);
        last_h_ind = Ns(1)-1;
        for j = 2:length(obj.mode_sequence)
            temp_h_inds(next_jump) = NaN;
            temp_h_inds(next_jump+1:next_jump+Ns(j)-1) = obj.h_inds(last_h_ind+1:last_h_ind+Ns(j)-1);
            next_jump = next_jump + Ns(j);
            last_h_ind = last_h_ind+Ns(j)-1;
        end
        obj = obj.addCost(cost, {reshape([temp_h_inds'; obj.x_inds(:,1:end-1); obj.u_inds],[],1); obj.x_inds(:,end)});
    ...
    function obj = addRobustInputConstraint(obj)
        ...
        for j = 2:length(obj.mode_sequence)
            temp_h_inds(next_jump) = NaN;
            temp_h_inds(next_jump+1:next_jump+N(j)-1) = obj.h_inds(last_h_ind+1:last_h_ind+N(j)-1);
            next_jump = next_jump + obj.N(j);
            last_h_ind = last_h_ind+N(j)-1;
        end
        obj = obj.addConstraint(constraint, {reshape([temp_h_inds'; obj.x_inds(:,1:end-1); obj.u_inds],[],1); obj.x_inds(:,end)});
    ...
    function [f,df] = forward_constraint_fun(obj,h,x0,x1,u)
        ...
        mode_num = 1;
      nX = obj.plant.getNumStates()-obj.plant.getNumDiscStates();
      [xdot,dxdot] = obj.plant.modes{mode_num}.dynamics(0,x0,u);
      f = x1 - x0 - h*xdot;
    ...
    function [f,df] = midpoint_constraint_fun(obj,h,x0,x1,u0,mode_num)
      nX = obj.plant.getNumStates()-obj.plant.getNumDiscStates();
      [xdot,dxdot] = obj.plant.modes{mode_num}.dynamics(0,.5*(x0+x1),u0);
      f = x1 - x0 - h*xdot;
        ...
        if any([y; xf] ~= obj.z_handle.data)
            ...
            switch obj.options.integration_method
                case RobustDirtranTrajectoryOptimization.FORWARD_EULER
                    ...
                    node_counter = 1; %Counts off number of nodes in sequence
                    mode_num = 1;
                    for k = 1:(N-1)
                        if (node_counter == N(mode_num))
                            %A for transition fun for Jump Riccati
                            [~,~,~,dxp] = obj.transition{mode_num}{1}(obj,obj.mode_sequence(mode_num),0,y((k-1)*(1+nX+nU)+1+(1:nX)),y((k-1)*(1+nX+nU)+1+nX+(1:nU)));             
                            A(:,:,k) = dx(:,2 +(1:nX));
                            
                            mode_num = mode_num+1;
                            node_counter = 1;
                        else
                            [~,dx,d2x] = obj.robust_dynamics(y((k-1)*(1+nX+nU)+1),y((k-1)*(1+nX+nU)+1+(1:nX)),y((k-1)*(1+nX+nU)+1+nX+(1:nU)),zeros(nW,1),obj.mode_sequence(mode_num));
                            A(:,:,k) = dx(:,1+(1:nX));
                            B(:,:,k) = dx(:,1+nX+(1:nU));
                            G(:,:,k) = dx(:,1+nX+nU+(1:nW));
                    ...
                    num_Ks = length(obj.h_inds);
                    P = obj.Qf;                   
                    dP = zeros(nX*nX,(num_Ks)*(1+nX+nU)+nX);
                    K = zeros(nU,nX,num_Ks);
                    dK = zeros(nU*nX,(num_Ks)*(1+nX+nU)+nX,num_Ks);
                    %dP = zeros(nX*nX,(N-1)*(1+nX+nU)+nX);
                    %K = zeros(nU,nX,N-1);
                    %dK = zeros(nU*nX,(N-1)*(1+nX+nU)+nX,N-1);
                    if(~constant_Ks)
                        %K = repmat(obj.constant_K,[1,1,num_Ks]);
                        K = repmat(obj.constant_K,[1,1,N-1]);
                    else
                    mode_num = length(obj.mode_sequence);
                    node_counter = obj.N(mode_num);
                    K_count = num_Ks;
                    for k = (N-1):-1:1
                        if (node_counter == 1) %Boundary of jump
                            P = A(:,:,k)'*P*A(:,:,k); %Jump Riccati                         
                        else
                            K(:,:,K_count) = (B(:,:,k).'*P*B(:,:,k)+obj.R)\(B(:,:,k).'*P*A(:,:,k));
                            ...
                            dK(:,:,K_count) = dKdP*dP;
                            dK(:,(k-1)*(1+nX+nU)+(1:(1+nX+nU)),K_count) = dK(:,(k-1)*(1+nX+nU)+(1:(1+nX+nU)),k) + dKdA*dA(:,:,k) + dKdB*dB(:,:,k);

                            dPdA = kron(eye(nX), (A(:,:,k)-B(:,:,k)*K(:,:,K_count))'*P) + kron((A(:,:,k)-B(:,:,k)*K(:,:,K_count))'*P, eye(nX))*comm(nX,nX);
                            dPdB = -kron(eye(nX), (A(:,:,k)-B(:,:,k)*K(:,:,K_count))'*P)*kron(K(:,:,K_count)', eye(nX)) - kron((A(:,:,k)-B(:,:,k)*K(:,:,K_count))'*P, eye(nX))*kron(eye(nX), K(:,:,K_count)')*comm(nX,nU);
                            dPdK = kron(eye(nX), K(:,:,K_count)'*obj.R) + kron(K(:,:,K_count)'*obj.R, eye(nX))*comm(nU,nX) - kron(eye(nX), (A(:,:,k)-B(:,:,k)*K(:,:,K_count))'*P)*kron(eye(nX), B(:,:,k)) - kron((A(:,:,k)-B(:,:,k)*K(:,:,K_count))'*P, eye(nX))*kron(B(:,:,k), eye(nX))*comm(nU,nX);
                            dPdP = kron((A(:,:,k)-B(:,:,k)*K(:,:,K_count))', (A(:,:,k)-B(:,:,k)*K(:,:,K_count))');

                            P = obj.Q + K(:,:,K_count).'*obj.R*K(:,:,k) + (A(:,:,k) - B(:,:,k)*K(:,:,K_count)).'*P*(A(:,:,k) - B(:,:,k)*K(:,:,K_count));
                            dP = dPdP*dP + dPdK*dK(:,:,K_count);
...
%                         dP(:,(k-1)*(1+nX+nU)+(1:(1+nX+nU))) = dP(:,(k-1)*(1+nX+nU)+(1:(1+nX+nU))) + dPdA*dA(:,:,k) + dPdB*dB(:,:,k);
    ...
    function [f,df,d2f] = robust_dynamics(obj,h,x,u,w,mode_num)
      % Euler integration of continuous dynamics
      if nargout == 1
          xdot = obj.plant.modes{mode_num}.dynamics_w(0,x,u,w);
          f = x + h*xdot;
      elseif nargout == 2
        [xdot,dxdot] = obj.plant.modes{mode_num}.dynamics_w(0,x,u,w);
        f = x + h*xdot;
        df = [xdot ... h
          ...
          [xdot,dxdot,d2xdot] = obj.plant.modes{mode_num}.dynamics_w(0,x,u,w);
          f = x + h*xdot;
          df = [xdot ... h
            eye(obj.nX) + h*dxdot(:,1+(1:obj.nX)) ... x0
            h*dxdot(:,1+obj.nX+(1:obj.nU)) ... u
    ...
    function xtraj = reconstructStateTrajectory(obj,z)
      ...
      xtraj = xtraj.setOutputFrame(obj.plant.getOutputFrame);
    end
