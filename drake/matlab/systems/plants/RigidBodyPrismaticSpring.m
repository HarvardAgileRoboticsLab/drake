classdef RigidBodyPrismaticSpring < RigidBodyForceElement
%  Adds prismatic linear springs, with 
%      force = k*(rest_length - current_angle),
%  to rigid body manipulators.  
  ...
  methods
    function [f_ext,df_ext] = computeSpatialForce(obj,manip,q,qd)
      ...
      torque = obj.k*(obj.rest_length - theta);
      ...
      wrench_on_child_in_child_frame = [zeros(3, 1); manip.body(obj.child_body).joint_axis * torque];
    ...
    function obj = updateForRemovedLink(obj,model,body_ind)
      if (obj.child_body == body_ind)
        error('removed joint with a prismatic spring.  need to handle this case');
      end
    end
    function [T,U] = energy(obj,manip,q,qd)
      T=0;
      theta = q(manip.body(obj.child_body).position_num);
      U = .5*obj.k*(obj.rest_length - theta)'*(obj.rest_length - theta);
    end
  end
  
  methods (Static)
    function [model,obj] = parseURDFNode(model,name,robotnum,node,options)
      obj = RigidBodyPrismaticSpring();
      obj.name = name;
...
%       assert(numel(model.body(obj.child_body).position_num)==1 && model.body(obj.child_body).pitch==0,'Prismatic springs are currently only supported for prismatic joints');
    end
  end
end
