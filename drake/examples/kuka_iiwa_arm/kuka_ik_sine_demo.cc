...
/// of time samples and @p t is an array of times in seconds).
    ...
    // coordinate.
      ...
      // safely bigger than e.g. a 200Hz input rate.
            int handled  = lcm_->handleTimeout(20);
          if (handled <= 0) {
  ...
  const int kNumTimesteps = 5; // must be even
  //double dt = 4;
  //double t[1] = {4};
  ...
  //if (i%2 == 1){
    Vector3d pos_end;
      //pos_end << 0.3*cos(i*PI/(double)kNumTimesteps), 0.3*sin(((double)i)*PI/(double) kNumTimesteps), .5;
    //pos_end << 0.3*cos(i*PI/(double)kNumTimesteps), 0.3*sin(((double)i)*PI/(double) kNumTimesteps), .5;
    pos_end << 0.6, 0, 0.325;

    printf("pos_end: %f, %f, %f\n",pos_end[0],pos_end[1],pos_end[2]);
      //Vector2d tspan(0, 3);//dt*((double)i-0.5), dt*((double)i+0.5)
      /*
      if (i == 1){
        tspan << dt*((double)i-0.5), dt*((double)i+0.5);
      }
      else{tspan << dt*((double)i-0.5), dt*((double)i+0.5);}  
      */
      //printf("tspan: %f, %f\n", tspan[0], tspan[1]);
      ...
      //if (i == 1){
        ...
        constraint_array.push_back(&wpc);
            std::cout << "pos_lb: " << pos_lb << std::endl;
            std::cout << "pos_ub: " << pos_ub << std::endl;

      //}
      /*
      if (i == 3){ 
        WorldPositionConstraint wpc1(&tree, tree.FindBodyIndex("iiwa_link_ee"),
                                  Vector3d::Zero(), pos_lb, pos_ub,
                                  tspan);
