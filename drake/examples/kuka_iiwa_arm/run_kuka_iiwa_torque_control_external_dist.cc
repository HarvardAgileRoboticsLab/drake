 ...
 public:
  ...
  void Run() {
    ...
    /*while (!time_initialized ||
      ...
      drake::WrenchVector<double> f_external;
      f_external.setRandom();
      
      external_wrenches.insert({tree_->FindBody("iiwa_link_ee"), f_external});
      ...
      Eigen::VectorXd torque_command = torque_ref + tree_->inverseDynamics(cache, external_wrenches, vd, false);
...
int main(int argc, const char* argv[]) {
  ...
  // 1st constraint, set straight up zero configuration
  ...
  WorldPositionConstraint wpc(&tree, tree.FindBodyIndex("iiwa_link_ee"),
                              Vector3d::Zero(), pos_lb, pos_ub, Vector2d(1, 5)); 

  // 3rd constraint, horizontal motion by setting joint constraint
  PostureConstraint pc2(&tree, Vector2d(6, 10)); 
  joint_lb(0) += 0.4;
  joint_ub(0) += 0.4;
  joint_lb(1) += 0.2;
  joint_ub(1) += 0.2;
  ...
  Eigen::VectorXd joint_ub2 = new_conf + VectorXd::Constant(7, 0.01);

  PostureConstraint pc4(&tree, Vector2d(20, 26));
  pc4.setJointLimits(joint_idx, joint_lb2, joint_ub2);

  // 7th constraint, move back to zero configuration
  joint_lb = zero_conf - VectorXd::Constant(7, 0.01);
  joint_ub = zero_conf + VectorXd::Constant(7, 0.01);
  PostureConstraint pc5(&tree, Vector2d(28, 32));
  pc5.setJointLimits(joint_idx, joint_lb, joint_ub);

  // 8th constraint, set the final Cartesian position
  pos_end << 0.35, -0.5, 0.225;
  pos_lb = pos_end - Vector3d::Constant(0.005);
  pos_ub = pos_end + Vector3d::Constant(0.005);
  WorldPositionConstraint wpc3(&tree, tree.FindBodyIndex("iiwa_link_ee"),
                              Vector3d::Zero(), pos_lb, pos_ub, Vector2d(34, 38));
