...
xtraj_phi = simulate(phi_sys, [t0 2.2], x00);
...
% need to do next...
% set this up so that it is functional with a hybrid system
% account for quaternion
% update things with git
% add switching logic to phi-tvlqr controller

% get A and B out of dynamics method
% just use Gk instead of mixed eye/Gn thing
...
plot3(xtraj_phi_eval(1,:), xtraj_phi_eval(2,:), xtraj_phi_eval(3,:), 'b');
plot3(xtraj_tvlqr_official_eval(1,:), xtraj_tvlqr_official_eval(2,:), xtraj_tvlqr_official_eval(3,:), 'k--'); hold off;
