...
options.dt = dt;
gait = 'TROT';
DC = 90;                % duty cycle for swing
DL = 0;                % percent "push" into the ground
LIFTAMP = 0.15;        % lift actuator motion (mm)     
SWINGAMP = 0.20;       % swing actuator motion (mm)
SAVE_FLAG = 0;
...
Vff = 0;        % feed forward voltage
fd = 0.06;             % drive frequency (kHz)
NCYC = 20;
tsim = NCYC/fd;
...
switch gait
    ...
    case 'TROT'
        vv = [(Vff/2)*sin(2*pi*fd*t+pi/2);                   % FLswing
            ...
            (Vff/2)*sin(2*pi*fd*t);              % FRlift
            (Vff/2)*sin(2*pi*fd*t - pi/2);                     % RRSwing
            (Vff/2)*sin(2*pi*fd*t)];             % RRLift
    case 'PRONK'
        vv = [(Vff/2)*sin(2*pi*fd*t + pi/2);            % FLswing
            (Vff/2)*sin(2*pi*fd*t);                     % FLlift
            (Vff/2)*sin(2*pi*fd*t - pi/2);              % RLSwing
...
% generate trajectory 
NPTS = 50;
traj = GenerateHueristicActTraj(gait, fd, DC, DL, NPTS);
tcyc = traj(1:end-1, 1); 
qcyc = traj(1:end-1,2:end);
...
%     plot(tt_sol, vv_sol_OL(i,:));
    plotyy(tt_sol, vv_sol_CL(i,:), tt_sol, vv(i,:));
%     plot();
...
for i = 1:nc
    ...
    xlabel('Time(s)'); ylabel('Foot X'); title(legs{i})
...
%     
%     subplot(nc,2, 2*i); hold on; 
%     xlabel('Time(s)'); ylabel('Foot Z'); title(legs{i})
%     plotyy(tt_sol, pfCL(:,3,i) - pfCL(1,3,i), tt_sol, xx_sol_CL(act_dof(2*i),:)); % pfCL(:,3,i)- pfCL(1,3,i));
%     
    axis tight; 
%     xlim([-2, 2])
%     ylim([-2, 2])
...
for i = 1:nc
    subplot(nc,2, 2*i-1); hold on; 
    xlabel('Time(s)'); ylabel('Foot X'); title(legs{i})
    plotyy(tt_sol, vv_sol_CL(2*i-1,:), tt_sol, xx_sol_CL(act_dof(2*i-1),:)); % pfCL(:,3,i)- pfCL(1,3,i));
%     axis tight;

    
    subplot(nc,2, 2*i); hold on; 
    xlabel('Time(s)'); ylabel('Foot Z'); title(legs{i})
    plotyy(tt_sol, vv_sol_CL(2*i,:), tt_sol, xx_sol_CL(act_dof(2*i),:)); % pfCL(:,3,i)- pfCL(1,3,i));    
%     axis tight; 
