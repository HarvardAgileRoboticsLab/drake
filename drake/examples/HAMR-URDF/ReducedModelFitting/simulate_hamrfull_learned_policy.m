...
HWebRequest = HamrWebRequest(hamrWact, hamr); 
...
% yy = ytraj.eval(tt); 
% 
% save('ppohamrfull', 'tt', 'xx', 'yy')

%% comparison 

hfull_data = load('ppohamrfull.mat');
ppo_dat = load('ppotraj.mat');
xhip = hfull_data.xx(HWebRequest.ihip, 1:end-1);

qppo = ppo_dat.q(:, 1:14)'; 
qdot_ppo = ppo_dat.q(:, 15:end)'; 

qsimple = [hfull_data.xx(1:6, 1:end-1); xhip(HWebRequest.perm, :)];
qdot_simple = [hfull_data.xx(HWebRequest.nq + (1:6), 1:end-1); ...
    xhip(HWebRequest.nu + HWebRequest.perm, :) ];


save('ppotraj_hfull', 'qsimple', 'qdot_simple');
% figure(1); clf;
% for i = 1:14
