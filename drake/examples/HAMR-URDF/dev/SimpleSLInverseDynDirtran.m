function [SL, xtraj,utraj,z,F,info] = SLSimpleInverseDynDirtran(tt, xfoot, ctraj, btraj)
%% Build Single Leg
...
SL = SLSimpleRBM(sl_urdf, options);
...
optimopt.integration_method = 1;                            % Forward Euler
...
% traj_opt = traj_opt.addStateConstraint(ConstantConstraint(zeros(nv,1)), N, nq+(1:nv));
...
x0 = SL.getInitialState();

traj_init.x = PPTrajectory(zoh([0, T], [x0, x0]));
traj_init.u = PPTrajectory(zoh(t_init, randn(nu,N)));
...
%% Costs and Constraints
    ...
    function [f, df] = objective(obj, x)
        ...
        for i = 1:N
            ...
            [xfooti, Jk] = obj.forwardKin(kinsoli, obj.findLinkId('FL2'), pf);
