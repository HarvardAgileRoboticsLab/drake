...
erri = []; 
for i = 1:N
    xip1_full = SL.update(t(i), x(1:(nq+nv), i), xi(nq+nv+(1:nu),i));     
    xi_simple = [xi(sfb_inputs,i) + SLSimple.q0; xi(nq+sfb_inputs,i)]; 
    xip1_simple = xi_simple + SLSimple.dynamics(t(i), xi_simple , tau(:,i))*SL.timestep;
    erri = xip1_full(2+sfb_inputs) - xip1_simple(3:4);
    f = f + (1/2)*erri'*erri; 
end
f
% f/N
...
% yyaxis left; plot(freq(ind0:ind1)*1e3, 2*Vfft(1,ind0:ind1))
% yyaxis right; plot(freq(ind0:ind1)*1e3, 2*yfft(1,ind0:ind1))
% 
% subplot(2,1,2); hold on; title('Lift')
% yyaxis left; plot(freq(ind0:ind1)*1e3, 2*Vfft(2,ind0:ind1))
...
% 
% inds = 1e3;
% data = iddata(yi(:,inds:end)', V(:, inds:end)', dt);
